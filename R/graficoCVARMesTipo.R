...
#'
#' @export
graficoCVARMesTipo <- function(baseSQLite, tipoCaso, numeroCaso, codModelo, inicioHorizonteGrafico, fimHorizonteGrafico,
  ...
  graficoCVaR <- ggplot(tib.resultadosCvarMes, aes(x = anoMes, y = cvar, colour = tamanhoCVAR, fill = tamanhoCVAR)) + 
    ...
    geom_text(aes(label = ifelse(cvar == maxCVAR, percent(cvar, accuracy = 0.1, scale = 100, suffix = "%", decimal.mark = ","), "")), 
              nudge_y = 0.04, show.legend = FALSE, hjust = 0.3, fontface = "bold", size = 5, family = "Montserrat") +
