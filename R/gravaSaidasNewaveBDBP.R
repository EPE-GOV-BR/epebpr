...
gravaSaidasNewaveBDBP <- function(pasta, pastaSaidas, conexao, tipoCaso, numeroCaso, codModelo) {
  ...
  DBI::dbExecute(conexao, paste0("DELETE FROM BPO_A06_SAIDA_HIDRO_NEWAVE
                             WHERE A01_TP_CASO = ", tipoCaso, " AND A01_NR_CASO = ", numeroCaso, " AND A01_CD_MODELO = ", codModelo))
  
  df.ree = DBI::dbGetQuery(conexao, paste0("SELECT A02_NR_REE, A02_TP_CALC_POTENCIA 
                                         FROM BPO_A02_REES 
  ...
  # Carrega arquivos
  df.energiaArmazenadaFinal <- leitorrmpe::leituraEnergiaArmazenadaFinalPercentual(pastaSaidas)
  df.geracaoHidroTotal <- leitorrmpe::leituraGeracaoHidroTotal(pastaSaidas)
  df.vertimentoTurbinavel <- leitorrmpe::leituraVertimentoFioDaguaTurbinavel(pastaSaidas)
  
  # calculo de potencia tipo 2: nao modula - considera a geracao hidraulica na ponta resultante da simulacao
  df.dadosUsinasCalculoTipo2 <- dplyr::full_join(df.energiaArmazenadaFinal, 
                                                 dplyr::filter(df.geracaoHidroTotal, patamar == 1), 
                                                 by = c("codREE", "serie", "anoMes")) %>% 
    dplyr::full_join(df.vertimentoTurbinavel, by = c("codREE", "serie", "anoMes")) %>%
    dplyr::inner_join(df.ree, by = c("codREE" = "A02_NR_REE")) %>%
    dplyr::filter(A02_TP_CALC_POTENCIA == 2) %>% 
    dplyr::inner_join(leitorrmpe::leituraDadosDuracaoPatamar(pasta), by = c("anoMes", "patamar")) %>%
    # calcula a geracao proporcional ao patamar de carga
    dplyr::mutate(geracao = geracao / duracaoPatamar, 
                  earmfp = ifelse(is.na(earmfp), 0, earmfp / 100)) %>% 
    dplyr::select(codREE, anoMes, serie, earmfp, geracao, vertimento)
  ...
  df.energiaArmazenadaFinal13 <- df.energiaArmazenadaFinal %>% dplyr::inner_join(df.ree, by = c("codREE" = "A02_NR_REE")) %>%
    dplyr::filter(A02_TP_CALC_POTENCIA != 2) %>% dplyr::select(-A02_TP_CALC_POTENCIA)
  
  df.vertimentoTurbinavel13 <- df.vertimentoTurbinavel %>% dplyr::inner_join(df.ree, by = c("codREE" = "A02_NR_REE")) %>%
    dplyr::filter(A02_TP_CALC_POTENCIA != 2) %>% dplyr::select(-A02_TP_CALC_POTENCIA)
  
  df.dadosUsinasCalculoDemaisTipos <- df.geracaoHidroTotal %>% 
    dplyr::inner_join(df.ree, by = c("codREE" = "A02_NR_REE")) %>%
    dplyr::filter(A02_TP_CALC_POTENCIA != 2) %>%
    dplyr::group_by(codREE, serie, anoMes) %>% 
    dplyr::summarise(geracao = sum(geracao)) %>% dplyr::ungroup() %>% 
    dplyr::full_join(df.energiaArmazenadaFinal13, by = c("codREE", "serie", "anoMes")) %>%
    dplyr::full_join(df.vertimentoTurbinavel13, by = c("codREE", "serie", "anoMes")) %>%
    dplyr::select(codREE, anoMes, serie, earmfp, geracao, vertimento)
  
  df.dadosUsinas <- rbind(df.dadosUsinasCalculoTipo2, df.dadosUsinasCalculoDemaisTipos) %>% 
    dplyr::mutate(tipoCaso = tipoCaso, numeroCaso = numeroCaso, codModelo = codModelo, submotorizacao = 0) %>% 
    dplyr::mutate(earmfp = ifelse(is.na(earmfp), 0, earmfp / 100)) %>% 
  ...
  DBI::dbWriteTable(conexao, "BPO_A06_SAIDA_HIDRO_NEWAVE", df.dadosUsinas, append = TRUE)
